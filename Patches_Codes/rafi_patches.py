...
def create_directories(base_dir, classes, subset):
    for z in classes:
        ...
        lbp_dir = os.path.join(base_dir, "final_patch_rafi_" + subset, z)
...
def process_images(base_dir, classes, subset):
    ...
    for z in classes:
        ...
        for f1 in files:
                ...
                temp1 = int(height/256)
                temp2 = int(width/256)
                crop_height = temp1*256
                crop_width = temp2*256
                ...
                while ((i+256) <= h):
                    ...
                    while ((j+256)<= w):
                        ...
                        img_lbp[:, :, :] = img_crop[i:i+256,j:j+256,:]
...
def process_images_2(base_dir, classes, subset):
    ...
    for z in classes:
        ...
        for f1 in files:
                ...
                img_lbp = np.zeros((256,256,3), np.uint8)
                img_patch = np.zeros((64,64,3), np.uint8)
                    ...
                    while ((j+256)<= w):
                        ...
                        if(isSaturated(img_lbp)>=limit):
                            ...
                            while((s+64) <= 256):
                                t = 0
                                while((t+64) <= 256):
                                    img_patch[:, :, :] = img_lbp[s:s+64,t:t+64,:]
                                    filename = Path(f1).stem+'_'+str(cluster)+"_"+str(k)+'.jpg'
                                    img_dir =  lbp_dir + '/' + filename
                                    cv2.imwrite(img_dir, img_patch)
                                    t = t + 64
                                    k=k+1
                                s = s + 64
                            cluster = cluster + 1
